<?php if(isset($aControllerDatas['paginationCommentaires'])) { $paginationCommentaires = $aControllerDatas['paginationCommentaires'];} // affiche les commentaires?>
<?php if(isset($aControllerDatas['article'])) { $article = $aControllerDatas['article']; } // affiche les éléments de l'article?>
...
<?php if(isset($aControllerDatas['commentaire_nbr'])) { $com = $aControllerDatas['commentaire_nbr']; } // affiche le nombre de commentaires?>
<?php if(isset($aControllerDatas['articlesTypesList'])) { $articlesTypesList = $aControllerDatas['articlesTypesList']; } // type de l'article?>
<?php if(isset($aControllerDatas['guildes'])) { $guildes = $aControllerDatas['guildes'];} // le pseudo de l'auteur et le nom de sa guilde?>
<?php if(isset($aControllerDatas['guildes2'])) { $guildes2 = $aControllerDatas['guildes2'];} // nom de la guilde de l'auteur pour un Guild Master?>

<div class="grid_16 background_blanc">
	<div>
		<div class="grid_5 icone_article">
			<img class="tournoi_soustitre_icone" style="margin-top:-25px; margin-bottom:20px; "src="<?php echo BASE_URL;?>/img/articles/art_participation_sans.jpg" title="Festival Fury-Game" alt="Festival Fury-Game" />
		</div>
		<div class=" principe_tournoi" style="margin-left:20px; margin-right:20px;">
			<div class="titre_articles"><?php echo $article['name']; ?></div>
			<div class="typearticle"><span>Janv, 20 </span> tags :<span><?php echo $articlesTypesList[$article['articles_type_id']]; ?></span> par <span>Djayk</span> dans <span> Articles</span>.</div>
			<div class="tournoi_description"><?php echo $article['content']; ?></div>
		</div>
	</div>
</div>
<div class="grid_16 background_blanc">
	...
	<div>					
		<div class="titrecommentaires titre_com" style="margin-top:50px; margin-bottom:40px;">Commentaire(s) : <?php  foreach($com as $k => $v) { echo $v;}?></div>
	</div>
	...
	<form class="form" enctype="multipart/form-data" method="post" action="<?php echo $_SERVER['REQUEST_URI']; ?>" id="Commentaires">
		<div class="row">
			...
			<div class="rowright">
				...
				<?php /** L'input de type hidden au dessus va permettre d'enregistrer le commentaire dans le bon article id et l'enregistre dans le champ article_id de la table commentaires, sans a le commentaire risque d'apparaitre dans tous les articles */ ?>
			...
			<div class="row">
				<button type="submit" class="bouton_valider" onclick="return confirm('Voulez vous vraiment ajouter ce membre?');">Ajouter un commentaire </button>
			</div>
		</div>
	</form>
	<?php }?>
